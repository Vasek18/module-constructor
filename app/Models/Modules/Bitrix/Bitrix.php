...
';
    ...
    // фильтры
    public function scopeUserCanSee($query, $userID){
        $user = User::find($userID);

        // собираем модули, на которые предоставили права
        $grantedModulesIDs = [];
        foreach (ModulesAccess::where('user_email', $user->email)->get() as $access){
            $grantedModulesIDs[] = $access->module_id;
        }

        if ($grantedModulesIDs){
            return $query->where('user_id', $userID)->orWhereIn('id', $grantedModulesIDs);
        } else{
