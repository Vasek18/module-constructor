...
$('#admin_options_dop_settings_window').on('show.bs.modal', function(event){
	...
	// выводим уже установленные значения
	modal_form.find("[name=option_height]").val(row.find("[name$=_height]").val());
	modal_form.find("[name=option_width]").val(row.find("[name$=_width]").val());
