...
@section('page')
    ...
    <a href="{{action('Modules\Bitrix\Infoblock\BitrixInfoblockController@create', [$module->id])}}"
       class="btn btn-primary">{{ trans('bitrix_data_storage.add_iblock_button') }}</a>
    <a href="{{action('Modules\Bitrix\Infoblock\BitrixInfoblockController@xml_ib_import', [$module->id])}}"
        ...
        <div class="list-group">
            @foreach($infoblocks as $infoblock)
                <div class="list-group-item clearfix infoblock deletion_wrapper">
                    <a href="{{ action('Modules\Bitrix\Infoblock\BitrixInfoblockController@show', [$module->id, $infoblock->id]) }}">
                        {{ trans('bitrix_data_storage.iblock') }} "{{$infoblock->name}}" ({{$infoblock->code}})
                    </a>
                    <a href="{{ action('Modules\Bitrix\Infoblock\BitrixInfoblockController@destroy', [$module->id, $infoblock->id]) }}"
