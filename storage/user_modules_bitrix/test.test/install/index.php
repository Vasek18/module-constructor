...
Class test_test extends CModule{
	...
	function __construct(){
		...
		$this->MODULE_NAME = Loc::getMessage("TEST_TEST_MODULE_NAME");
		$this->MODULE_DESCRIPTION = Loc::getMessage("TEST_TEST_MODULE_DESC");

		$this->PARTNER_NAME = getMessage("TEST_TEST_PARTNER_NAME");
		$this->PARTNER_URI = getMessage("TEST_TEST_PARTNER_URI");
	...
	function createIblockType(){
		...
		$iblockType = "test_test_iblock_type";
		...
		if (!$ar_iblock_type = $db_iblock_type->Fetch()){
			$arFieldsIBT = Array(
				...
				'LANG'     => Array(
					'en' => Array(
						'NAME' => Loc::getMessage("TEST_TEST_IBLOCK_TYPE_NAME_EN"),
					),
					'ru' => Array(
						'NAME' => Loc::getMessage("TEST_TEST_IBLOCK_TYPE_NAME_RU"),
					)
				)
			);
			...
			if (!$resIBT){
				$DB->Rollback();
				$APPLICATION->ThrowException(Loc::getMessage("TEST_TEST_IBLOCK_TYPE_ALREADY_EXISTS"));
			}else{
				$DB->Commit();
	...
	function removeIblockType(){
		...
		if (!CIBlockType::Delete($iblockType)){
			$DB->Rollback();
			$APPLICATION->ThrowException(Loc::getMessage("TEST_TEST_IBLOCK_TYPE_DELETION_ERROR"));
		}
		$DB->Commit();
	}
	...
	function createIblock($params){
		...
		if ($ar_resIBE = $resIBE->Fetch()){
			$APPLICATION->ThrowException(Loc::getMessage("TEST_TEST_IBLOCK_ALREADY_EXISTS"));
	...
	function DoInstall(){
		...
		if ($this->isVersionD7()){
			...
			$APPLICATION->ThrowException(Loc::getMessage("TEST_TEST_INSTALL_ERROR_VERSION"));
		}

		$APPLICATION->IncludeAdminFile(Loc::getMessage("TEST_TEST_INSTALL"), $this->GetPath()."/install/step.php");
	}
	...
	function DoUninstall(){
		...
		$APPLICATION->IncludeAdminFile(Loc::getMessage("TEST_TEST_UNINSTALL"), $this->GetPath()."/install/unstep.php");
	}
}
?>